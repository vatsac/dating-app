...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        private async Task<IEnumerable<int>> GetUserLikes(int id, bool likers)
        {
            ...
            .Include(x => x.LikesLiker)
            .Include(x => x.LikesLikee)
            .FirstOrDefaultAsync(u => u.Id ==id);

            if(likers)
            {
                return user.LikesLiker.Where(u => u.LikeeId == id).Select(i => i.LikerId);
            }
            else 
            {
                return user.LikesLikee.Where(u => u.LikerId == id).Select(i => i.LikeeId);
            }
        }
